...
应用名称为「SuQCanvas 桌面版」，当前版本 1.4.3。安装程序为 `release/desktop/SuQCanvas-Desktop-1.4.3-Setup-x64.exe`，支持选择安装位置、桌面快捷方式和 `.sqcanvas` 文件关联。
...
- **服务器项目**：首页切换到「局域网项目」，在页内连接卡片填写 `ws://服务器IP:8790` 或 `wss://域名/lan-ws` 和协作名称。连接后可查看、刷新项目或退出连接，断开连接不影响本地编辑。
