# 云端同步

<cite>
**本文引用的文件**
- [cloudSync.ts](file://src/sync/cloudSync.ts)
- [supabaseClient.ts](file://src/sync/supabaseClient.ts)
- [ossClient.ts](file://src/sync/ossClient.ts)
- [ossClientImpl.ts](file://src/sync/ossClientImpl.ts)
- [ossConfig.ts](file://src/sync/ossConfig.ts)
- [assetCloudUpload.ts](file://src/sync/assetCloudUpload.ts)
- [uploadStore.ts](file://src/store/uploadStore.ts)
- [schema.sql](file://supabase/schema.sql)
- [index.ts](file://supabase/functions/oss-sts/index.ts)
- [db.ts](file://src/db/db.ts)
- [authStore.ts](file://src/store/authStore.ts)
- [projectStore.ts](file://src/store/projectStore.ts)
- [types.ts](file://src/types.ts)
- [buildMode.ts](file://src/buildMode.ts)
</cite>

## 更新摘要
**所做更改**
- 新增OSS客户端分片上传功能，支持大于10MB的大文件传输
- 增强进度报告机制，提供实时上传进度反馈
- 改进错误处理，`upsertAssetMetaToCloud`函数现在返回布尔值表示操作结果
- 完善上传管道中的错误传播机制
- 新增资产云上传协调器，统一管理上传流程

## 目录
1. [简介](#简介)
2. [项目结构](#项目结构)
3. [核心组件](#核心组件)
4. [架构总览](#架构总览)
5. [详细组件分析](#详细组件分析)
6. [依赖关系分析](#依赖关系分析)
7. [性能与可靠性](#性能与可靠性)
8. [故障排查指南](#故障排查指南)
9. [配置指南](#配置指南)
10. [使用示例](#使用示例)
11. [结论](#结论)

## 简介
本技术文档围绕"云端同步"能力，系统性阐述 Supabase 集成（数据库、认证、行级安全）、阿里云 OSS 对象存储客户端（上传、下载、签名 URL、凭证刷新）、云同步策略（数据版本控制、冲突解决、增量思路）与安全考量（访问控制、权限隔离、密钥管理），并提供可操作的配置说明与自定义扩展示例。系统支持在线版与局域网版两种构建目标：在线版启用 Supabase 与 OSS；局域网版仅本地 Dexie 存储与 LAN 同步。

**更新** 本次更新增强了OSS客户端的分片上传功能和进度报告机制，改进了错误处理流程，为大规模文件传输提供更好的用户体验和可靠性保障。

## 项目结构
云端同步相关代码主要分布在以下模块：
- 同步编排层：src/sync/cloudSync.ts
- 资产上传协调器：src/sync/assetCloudUpload.ts
- Supabase 客户端封装：src/sync/supabaseClient.ts
- 阿里云 OSS 客户端封装与实现：src/sync/ossClient.ts、src/sync/ossClientImpl.ts、src/sync/ossConfig.ts
- 上传状态管理：src/store/uploadStore.ts
- 数据库模式与 RLS 策略：supabase/schema.sql
- STS 临时凭证签发服务：supabase/functions/oss-sts/index.ts
- 本地持久化与清理：src/db/db.ts
- 认证状态与登录流程：src/store/authStore.ts
- 项目生命周期与自动保存：src/store/projectStore.ts
- 类型定义：src/types.ts
- 构建目标开关：src/buildMode.ts

```mermaid
graph TB
UI["应用界面"] --> PS["项目状态(projectStore)"]
PS --> CS["云端同步(cloudSync)"]
CS --> SB["Supabase 客户端(supabaseClient)"]
CS --> DBX["本地数据库(db.ts)"]
PS --> OS["OSS 客户端(ossClient/ossClientImpl)"]
OS --> OSS["阿里云 OSS"]
SB --> SUPA["Supabase 后端"]
SUPA --> |RLS| TBL["projects/assets 表"]
OS --> |STS| FUNC["OSS STS 函数(index.ts)"]
ACU["资产上传(assetCloudUpload)"] --> OS
ACU --> CS
US["上传状态(uploadStore)"] --> ACU
```

**图表来源**
- [projectStore.ts:1-230](file://src/store/projectStore.ts#L1-L230)
- [cloudSync.ts:1-169](file://src/sync/cloudSync.ts#L1-L169)
- [assetCloudUpload.ts:1-45](file://src/sync/assetCloudUpload.ts#L1-L45)
- [uploadStore.ts:1-85](file://src/store/uploadStore.ts#L1-L85)
- [supabaseClient.ts:1-18](file://src/sync/supabaseClient.ts#L1-L18)
- [ossClient.ts:1-67](file://src/sync/ossClient.ts#L1-L67)
- [ossClientImpl.ts:1-154](file://src/sync/ossClientImpl.ts#L1-L154)
- [schema.sql:1-90](file://supabase/schema.sql#L1-L90)
- [index.ts:1-111](file://supabase/functions/oss-sts/index.ts#L1-L111)

**章节来源**
- [projectStore.ts:1-230](file://src/store/projectStore.ts#L1-L230)
- [cloudSync.ts:1-169](file://src/sync/cloudSync.ts#L1-L169)
- [assetCloudUpload.ts:1-45](file://src/sync/assetCloudUpload.ts#L1-L45)
- [uploadStore.ts:1-85](file://src/store/uploadStore.ts#L1-L85)
- [supabaseClient.ts:1-18](file://src/sync/supabaseClient.ts#L1-L18)
- [ossClient.ts:1-67](file://src/sync/ossClient.ts#L1-L67)
- [ossClientImpl.ts:1-154](file://src/sync/ossClientImpl.ts#L1-L154)
- [schema.sql:1-90](file://supabase/schema.sql#L1-L90)
- [index.ts:1-111](file://supabase/functions/oss-sts/index.ts#L1-L111)
- [db.ts:1-69](file://src/db/db.ts#L1-L69)
- [authStore.ts:1-104](file://src/store/authStore.ts#L1-L104)
- [types.ts:1-112](file://src/types.ts#L1-L112)
- [buildMode.ts:1-8](file://src/buildMode.ts#L1-L8)

## 核心组件
- 云端同步编排器（cloudSync.ts）
  - 负责将本地项目/素材与云端 Supabase 进行读写映射，处理用户身份校验、数据转换、错误提示。
  - **更新** `upsertAssetMetaToCloud`函数现在返回布尔值表示操作成功与否，支持更好的错误传播。
- 资产上传协调器（assetCloudUpload.ts）
  - **新增** 统一管理素材上传流程，协调OSS上传和元数据同步，提供进度回调和错误处理。
- Supabase 客户端（supabaseClient.ts）
  - 基于环境变量初始化 Supabase 客户端，提供 isCloudConfigured 判断是否启用云端能力。
- 阿里云 OSS 客户端（ossClient.ts + ossClientImpl.ts + ossConfig.ts）
  - 对外暴露统一接口：上传/下载媒体与缩略图、获取签名 URL、检查配置是否就绪。
  - **更新** 支持大于10MB文件的分片上传，提供实时进度反馈。
  - 内部通过 STS 或 AK/SK 获取临时/长期凭证，维护客户端实例与过期时间，支持自动刷新。
- 上传状态管理（uploadStore.ts）
  - **新增** 管理上传任务的状态跟踪，提供进度节流和错误恢复机制。
- 数据库模式与 RLS（schema.sql）
  - 定义 projects 与 assets 表结构，启用行级安全策略，确保按 user_id 隔离数据。
- STS 凭证服务（supabase/functions/oss-sts/index.ts）
  - 在 Supabase Edge Functions 中调用阿里云 STS AssumeRole，返回临时凭证供前端直传 OSS。
- 本地存储与清理（db.ts）
  - 使用 Dexie 维护本地项目与素材，提供垃圾回收逻辑，避免孤立资源占用空间。
- 认证与项目状态（authStore.ts, projectStore.ts）
  - 管理登录态、游客模式切换；根据登录态选择云端或本地数据源；实现自动保存与列表同步。

**章节来源**
- [cloudSync.ts:1-169](file://src/sync/cloudSync.ts#L1-L169)
- [assetCloudUpload.ts:1-45](file://src/sync/assetCloudUpload.ts#L1-L45)
- [uploadStore.ts:1-85](file://src/store/uploadStore.ts#L1-L85)
- [supabaseClient.ts:1-18](file://src/sync/supabaseClient.ts#L1-L18)
- [ossClient.ts:1-67](file://src/sync/ossClient.ts#L1-L67)
- [ossClientImpl.ts:1-154](file://src/sync/ossClientImpl.ts#L1-L154)
- [ossConfig.ts:1-20](file://src/sync/ossConfig.ts#L1-L20)
- [schema.sql:1-90](file://supabase/schema.sql#L1-L90)
- [index.ts:1-111](file://supabase/functions/oss-sts/index.ts#L1-L111)
- [db.ts:1-69](file://src/db/db.ts#L1-L69)
- [authStore.ts:1-104](file://src/store/authStore.ts#L1-L104)
- [projectStore.ts:1-230](file://src/store/projectStore.ts#L1-L230)

## 架构总览
系统采用"双通道"数据流：
- 项目元数据（nodes/edges/viewport）通过 Supabase 的 projects 表进行云端持久化；未登录时落盘 Dexie。
- 媒体文件（图片/视频/PDF 等）通过阿里云 OSS 存储，assets 表仅保存元数据（名称、MIME、大小、kind、OSS key）。
- 认证由 Supabase Auth 管理，行级安全策略保证每个用户只能访问自己的数据。
- 构建目标决定功能开关：在线版启用 Supabase/OSS；局域网版禁用并回退到本地/LAN 同步。

**更新** 新增了智能分片上传机制，对于大于10MB的文件自动启用分片上传，提供更好的大文件传输体验。

```mermaid
sequenceDiagram
participant U as "用户"
participant P as "项目状态(projectStore)"
participant C as "云端同步(cloudSync)"
participant A as "资产上传(assetCloudUpload)"
participant S as "Supabase 客户端"
participant O as "OSS 客户端"
participant F as "STS 函数"
participant B as "阿里云 OSS"
U->>P : 打开/编辑项目
P->>C : 读取/保存项目
alt 已登录
C->>S : upsert/fetch projects
S-->>C : 成功/失败
else 未登录
C->>P : 从本地 Dexie 读取/写入
end
U->>A : 上传媒体文件
A->>O : 上传(带进度回调)
alt 文件大小 > 10MB
O->>B : multipartUpload(分片上传)
else 小文件
O->>B : put(直接上传)
end
O->>F : 请求临时凭证(STS)
F-->>O : 返回临时凭证
B-->>O : 返回结果
A->>C : 同步元数据(返回布尔值)
C-->>A : 成功/失败
```

**图表来源**
- [projectStore.ts:1-230](file://src/store/projectStore.ts#L1-L230)
- [cloudSync.ts:1-169](file://src/sync/cloudSync.ts#L1-L169)
- [assetCloudUpload.ts:1-45](file://src/sync/assetCloudUpload.ts#L1-L45)
- [uploadStore.ts:1-85](file://src/store/uploadStore.ts#L1-L85)
- [supabaseClient.ts:1-18](file://src/sync/supabaseClient.ts#L1-L18)
- [ossClient.ts:1-67](file://src/sync/ossClient.ts#L1-L67)
- [ossClientImpl.ts:1-154](file://src/sync/ossClientImpl.ts#L1-L154)
- [index.ts:1-111](file://supabase/functions/oss-sts/index.ts#L1-L111)

## 详细组件分析

### Supabase 集成（数据库、认证、行级安全）
- 客户端初始化与可用性判断
  - 通过环境变量注入 Supabase URL 与匿名 Key，仅在在线构建下创建客户端。
  - 提供 isCloudConfigured 用于上层判断是否启用云端能力。
- 认证与会话
  - 认证状态由 authStore 管理，监听会话变化，切换游客/登录态，并在切换时重置项目状态，防止云端与本地数据串写。
- 数据模型与 RLS
  - projects 表：id(user uuid)、user_id、name、graph(jsonb)、viewport(jsonb)、created_at、updated_at。
  - assets 表：id(text)、user_id、name、mime、size、kind、oss_key、oss_thumb_key、has_thumbnail、created_at。
  - 启用行级安全策略，仅允许 authenticated 用户读写 own 数据（user_id = auth.uid()）。
- 云端 CRUD
  - 项目：列表查询、单条查询、插入/更新（upsert）、重命名、删除。
  - 素材元数据：upsert、删除、按 id 列表查询。
  - **更新** 所有写操作均附带当前用户 ID，读操作受 RLS 保护，`upsertAssetMetaToCloud`现在返回布尔值表示操作结果。

```mermaid
classDiagram
class CloudSync {
+isCloudAuthed() Promise~boolean~
+fetchCloudProjects() Promise~CloudProject[]~
+fetchCloudProject(id) Promise~CloudProject|null~
+upsertProjectToCloud(p) Promise~boolean~
+updateProjectNameInCloud(id,name) void
+deleteProjectFromCloud(id) void
+syncProjectList() Promise~ProjectRecord[]~
+loadProjectBest(id) Promise~ProjectRecord|null~
+upsertAssetMetaToCloud(meta,ossKey,thumbKey?) Promise~boolean~
+deleteAssetFromCloud(id) void
+fetchCloudAssets(ids) Promise~CloudAsset[]~
}
class AssetCloudUpload {
+cloudUploadApplies() Promise~boolean~
+runAssetCloudUpload(meta,blob,thumbnail,onProgress) Promise~boolean~
}
class UploadStore {
+uploads : Record~string, UploadStatus~~
+runCloudUpload(assetId) Promise~boolean~
}
CloudSync --> AssetCloudUpload : "被调用"
AssetCloudUpload --> UploadStore : "状态管理"
```

**图表来源**
- [cloudSync.ts:1-169](file://src/sync/cloudSync.ts#L1-L169)
- [assetCloudUpload.ts:1-45](file://src/sync/assetCloudUpload.ts#L1-L45)
- [uploadStore.ts:1-85](file://src/store/uploadStore.ts#L1-L85)
- [supabaseClient.ts:1-18](file://src/sync/supabaseClient.ts#L1-L18)

**章节来源**
- [supabaseClient.ts:1-18](file://src/sync/supabaseClient.ts#L1-L18)
- [schema.sql:1-90](file://supabase/schema.sql#L1-L90)
- [authStore.ts:1-104](file://src/store/authStore.ts#L1-L104)
- [cloudSync.ts:1-169](file://src/sync/cloudSync.ts#L1-L169)

### 阿里云 OSS 客户端（上传、下载、签名 URL、凭证刷新）
- 配置检测与动态加载
  - 通过 VITE_BUILD_TARGET 与 VITE_OSS_* 环境变量判断是否启用 OSS。
  - 在线构建且配置齐全时才加载 ossClientImpl，否则返回空实现，避免引入 ali-oss。
- 凭证获取与刷新
  - 优先通过 STS 函数获取临时凭证（含 securityToken 与 expiration），未配置则回退为 AK/SK。
  - 客户端缓存实例，并在过期前 5 分钟主动刷新；若使用 STS，开启 refreshSTSToken 回调与定时刷新间隔。
- **更新** 文件操作增强
  - **分片上传**：对于大于10MB的文件，自动使用multipartUpload进行分片上传，提高大文件传输的稳定性和效率。
  - **进度回调**：分片上传提供实时进度反馈，支持细粒度的上传进度监控。
  - 上传媒体与缩略图：put(key, blob)，key 由 assetKey/thumbKey 生成。
  - 下载媒体与缩略图：get(key) 并统一转换为 Blob。
  - 获取可访问 URL：signatureUrl(key, { expires: 3600 })，默认 1 小时有效。
- 错误处理
  - 下载/URL 生成失败时记录警告并返回空值，避免阻断主流程。

```mermaid
flowchart TD
Start(["开始"]) --> CheckCfg["检查 OSS 配置"]
CheckCfg --> |未配置| ReturnNull["返回 null"]
CheckCfg --> |已配置| GetCred["获取凭证(STS 或 AK/SK)"]
GetCred --> NewClient["创建/复用 OSS 客户端"]
NewClient --> Op{"操作类型"}
Op --> |上传| CheckSize{"文件大小 > 10MB?"}
CheckSize --> |是| Multipart["multipartUpload(分片上传)"]
CheckSize --> |否| Put["put(直接上传)"]
Multipart --> Progress["进度回调(onProgress)"]
Put --> End(["结束"])
Progress --> End
Op --> |下载| Get["get(key) -> Blob"]
Op --> |URL| Sig["signatureUrl(key,{expires})"]
Get --> End
Sig --> End
```

**图表来源**
- [ossConfig.ts:1-20](file://src/sync/ossConfig.ts#L1-L20)
- [ossClientImpl.ts:1-154](file://src/sync/ossClientImpl.ts#L1-L154)
- [ossClient.ts:1-67](file://src/sync/ossClient.ts#L1-L67)

**章节来源**
- [ossConfig.ts:1-20](file://src/sync/ossConfig.ts#L1-L20)
- [ossClientImpl.ts:1-154](file://src/sync/ossClientImpl.ts#L1-L154)
- [ossClient.ts:1-67](file://src/sync/ossClient.ts#L1-L67)

### 资产上传协调器（新增功能）
- **新增** 统一的上传流程管理
  - 协调OSS文件上传和元数据同步，确保两者的一致性。
  - 提供进度回调机制，支持实时上传进度反馈。
  - 实现错误处理和重试机制，提高上传可靠性。
- **更新** 智能分片上传
  - 自动检测文件大小，对大于10MB的文件启用分片上传。
  - 分片大小设置为2MB，并行度为4，优化大文件传输性能。
  - 提供细粒度的进度回调，支持10%~90%的主文件上传进度。
- **更新** 增强的错误处理
  - 缩略图上传失败不影响主文件上传，提高容错性。
  - 元数据同步失败时返回false，支持上层错误处理。
  - 统一的错误日志记录，便于问题诊断。

**章节来源**
- [assetCloudUpload.ts:1-45](file://src/sync/assetCloudUpload.ts#L1-L45)
- [ossClientImpl.ts:74-100](file://src/sync/ossClientImpl.ts#L74-L100)

### 上传状态管理（新增功能）
- **新增** 上传任务状态跟踪
  - 管理多个并发上传任务的状态，支持进度显示和错误处理。
  - 实现进度节流机制，避免高频更新导致的性能问题。
  - 提供上传完成后的自动清理机制。
- **更新** 智能状态管理
  - 上传中途关闭页面时的状态修复，避免残留的uploading状态。
  - 支持重试机制，失败的上传任务可以重新尝试。
  - 与UI组件集成，提供实时的上传状态反馈。

**章节来源**
- [uploadStore.ts:1-85](file://src/store/uploadStore.ts#L1-L85)

### 云同步策略（版本控制、冲突解决、增量同步）
- 版本控制
  - 项目记录包含 updated_at 字段，每次保存更新该时间戳；列表按更新时间倒序展示最新项目。
  - 素材记录包含 created_at，便于后续扩展审计与回溯。
- 冲突解决
  - 当前实现以"最后写入者胜"的策略：后保存的项目覆盖先前版本（upsert by id）。
  - 如需乐观锁/版本号冲突检测，可在服务端增加 version 字段并在更新时校验。
- 增量同步
  - 当前为全量保存项目 graph（nodes/edges/viewport），适合中小型画布。
  - 可扩展为基于节点/边差异的增量同步，减少网络负载。
- 数据源隔离
  - 已登录用户：项目列表与加载均来自云端；未登录/游客：仅本地 Dexie。
  - 切换登录态会重置项目状态，避免跨环境数据串写。

**章节来源**
- [schema.sql:1-90](file://supabase/schema.sql#L1-L90)
- [cloudSync.ts:1-169](file://src/sync/cloudSync.ts#L1-L169)
- [projectStore.ts:1-230](file://src/store/projectStore.ts#L1-L230)

### 安全考虑（访问控制、数据加密、权限管理）
- 认证与会话
  - 使用 Supabase Auth 管理用户登录态，监听会话变化，退出时清理本地状态。
- 行级安全（RLS）
  - projects 与 assets 表启用 RLS，限制 authenticated 用户仅能读写 own 数据（user_id = auth.uid()）。
- 密钥与凭证
  - 前端不直接持有高权限 AK/SK；通过 STS 函数获取短期凭证，降低泄露风险。
  - STS 函数部署在 Supabase Edge Functions，凭据以 Secrets 形式管理。
- 传输安全
  - OSS 客户端使用 secure: true，强制 HTTPS 传输。
- 最小权限原则
  - RAM 角色仅授予必要的 sts:AssumeRole 与 OSS 读写权限。

**章节来源**
- [schema.sql:1-90](file://supabase/schema.sql#L1-L90)
- [index.ts:1-111](file://supabase/functions/oss-sts/index.ts#L1-L111)
- [ossClientImpl.ts:1-154](file://src/sync/ossClientImpl.ts#L1-L154)
- [authStore.ts:1-104](file://src/store/authStore.ts#L1-L104)

## 依赖关系分析
- 构建目标影响依赖注入
  - buildMode.ts 定义 IS_LAN_BUILD/IS_ONLINE_BUILD，用于条件编译移除无关依赖。
  - supabaseClient.ts 仅在在线构建下创建 Supabase 客户端。
  - ossClient.ts 仅在在线构建下动态导入 ossClientImpl，避免引入 ali-oss。
- 运行时依赖
  - cloudSync.ts 依赖 supabaseClient.ts 与 db.ts。
  - **更新** assetCloudUpload.ts 依赖 cloudSync.ts 和 ossClient.ts，协调上传流程。
  - **更新** uploadStore.ts 依赖 assetCloudUpload.ts，管理上传状态。
  - projectStore.ts 依赖 cloudSync.ts、authStore.ts、lanClient.ts（LAN 同步，不在本文范围）。
  - ossClientImpl.ts 依赖 ossConfig.ts 与 ali-oss。

```mermaid
graph LR
BM["buildMode.ts"] --> SC["supabaseClient.ts"]
BM --> OC["ossClient.ts"]
OC --> OCI["ossClientImpl.ts"]
OCI --> CFG["ossConfig.ts"]
CS["cloudSync.ts"] --> SC
CS --> DB["db.ts"]
ACU["assetCloudUpload.ts"] --> CS
ACU --> OC
US["uploadStore.ts"] --> ACU
PS["projectStore.ts"] --> CS
PS --> AS["authStore.ts"]
```

**图表来源**
- [buildMode.ts:1-8](file://src/buildMode.ts#L1-L8)
- [supabaseClient.ts:1-18](file://src/sync/supabaseClient.ts#L1-L18)
- [ossClient.ts:1-67](file://src/sync/ossClient.ts#L1-L67)
- [ossClientImpl.ts:1-154](file://src/sync/ossClientImpl.ts#L1-L154)
- [ossConfig.ts:1-20](file://src/sync/ossConfig.ts#L1-L20)
- [cloudSync.ts:1-169](file://src/sync/cloudSync.ts#L1-L169)
- [assetCloudUpload.ts:1-45](file://src/sync/assetCloudUpload.ts#L1-L45)
- [uploadStore.ts:1-85](file://src/store/uploadStore.ts#L1-L85)
- [db.ts:1-69](file://src/db/db.ts#L1-L69)
- [projectStore.ts:1-230](file://src/store/projectStore.ts#L1-L230)
- [authStore.ts:1-104](file://src/store/authStore.ts#L1-L104)

**章节来源**
- [buildMode.ts:1-8](file://src/buildMode.ts#L1-L8)
- [supabaseClient.ts:1-18](file://src/sync/supabaseClient.ts#L1-L18)
- [ossClient.ts:1-67](file://src/sync/ossClient.ts#L1-L67)
- [ossClientImpl.ts:1-154](file://src/sync/ossClientImpl.ts#L1-L154)
- [ossConfig.ts:1-20](file://src/sync/ossConfig.ts#L1-L20)
- [cloudSync.ts:1-169](file://src/sync/cloudSync.ts#L1-L169)
- [assetCloudUpload.ts:1-45](file://src/sync/assetCloudUpload.ts#L1-L45)
- [uploadStore.ts:1-85](file://src/store/uploadStore.ts#L1-L85)
- [db.ts:1-69](file://src/db/db.ts#L1-L69)
- [projectStore.ts:1-230](file://src/store/projectStore.ts#L1-L230)
- [authStore.ts:1-104](file://src/store/authStore.ts#L1-L104)

## 性能与可靠性
- 客户端缓存与凭证刷新
  - OSS 客户端实例缓存，避免重复初始化；在过期前 5 分钟刷新，减少中断概率。
  - STS 模式下设置 refreshSTSTokenInterval 定期刷新，提升长连接稳定性。
- **更新** 网络优化增强
  - **分片上传**：对于大于10MB的文件自动启用分片上传，提高大文件传输的稳定性和成功率。
  - **进度回调**：提供实时进度反馈，改善用户体验。
  - 合理设置签名 URL 过期时间，兼顾安全性与播放体验。
  - 对大文件上传可考虑分片上传（需扩展实现），结合断点续传提升成功率。
- 数据一致性
  - 项目保存为全量覆盖，简单可靠；如需更高并发协作，建议引入版本号与冲突合并策略。
  - **更新** 上传流程确保OSS文件和元数据的一致性，任何一步失败都会回滚整个操作。
- 本地清理
  - 提供 GC 逻辑，标记并清理长时间未被项目引用的孤立素材，释放浏览器存储空间。
- **更新** 错误处理增强
  - 改进的错误传播机制，确保上传管道中的错误能够正确向上层传递。
  - 提供详细的错误日志，便于问题诊断和调试。

## 故障排查指南
- 无法登录或无云端能力
  - 检查环境变量是否配置了 Supabase URL 与匿名 Key；确认在线构建目标。
  - 参考认证错误翻译逻辑，定位邮箱未验证、频率限制等问题。
- 项目列表为空或无法加载
  - 检查 RLS 策略是否正确启用；确认 user_id 关联正确。
  - 查看云端拉取失败的日志提示。
- **更新** 媒体文件上传失败
  - 检查 OSS 配置是否齐全（region、bucket、AK 或 STS URL）。
  - 确认 STS 函数可用且返回正常；检查 CORS 与鉴权头。
  - **分片上传问题**：检查文件大小是否超过10MB阈值，确认分片上传配置。
  - **进度回调问题**：检查onProgress回调是否正确实现，确认进度节流机制。
  - **错误处理问题**：检查`upsertAssetMetaToCloud`返回值，确认错误传播机制。
  - 下载失败会记录警告并返回空值，请检查网络与权限。
- 自动保存失败
  - 关注保存状态变为 error 时的错误日志；检查网络与云端可达性。
- **新增** 上传状态异常
  - 检查uploadStore中的上传状态是否正确更新。
  - 确认进度节流机制正常工作，避免频繁状态更新。
  - 检查上传完成后的自动清理逻辑是否正常执行。

**章节来源**
- [authStore.ts:1-104](file://src/store/authStore.ts#L1-L104)
- [cloudSync.ts:1-169](file://src/sync/cloudSync.ts#L1-L169)
- [assetCloudUpload.ts:1-45](file://src/sync/assetCloudUpload.ts#L1-L45)
- [uploadStore.ts:1-85](file://src/store/uploadStore.ts#L1-L85)
- [ossClientImpl.ts:1-154](file://src/sync/ossClientImpl.ts#L1-L154)
- [index.ts:1-111](file://supabase/functions/oss-sts/index.ts#L1-L111)

## 配置指南
- 环境变量（前端）
  - Supabase：VITE_SUPABASE_URL、VITE_SUPABASE_ANON_KEY
  - OSS：VITE_OSS_REGION、VITE_OSS_BUCKET、VITE_OSS_ACCESS_KEY_ID、VITE_OSS_ACCESS_KEY_SECRET、VITE_OSS_STS_URL
  - 构建目标：VITE_BUILD_TARGET（lan 或 online）
- Supabase 函数（STS）
  - 部署到 Supabase Edge Functions，配置 Secrets：ALIYUN_AK_ID、ALIYUN_AK_SECRET、ALIYUN_ROLE_ARN
  - 函数返回临时凭证，前端通过 fetch 调用并传入 Authorization/apikey 头（可选）
- 数据库初始化
  - 执行 schema.sql 创建表、索引、触发器与 RLS 策略
- 网络优化
  - 确保 CDN/边缘节点就近访问 Supabase 与 OSS
  - 合理设置签名 URL 过期时间与缓存策略
- **更新** 分片上传配置
  - 分片大小：2MB（可根据网络状况调整）
  - 并行度：4（可根据服务器性能调整）
  - 阈值：10MB（小于此值的文件直接上传，大于等于此值使用分片上传）

**章节来源**
- [supabaseClient.ts:1-18](file://src/sync/supabaseClient.ts#L1-L18)
- [ossConfig.ts:1-20](file://src/sync/ossConfig.ts#L1-L20)
- [ossClientImpl.ts:1-154](file://src/sync/ossClientImpl.ts#L1-L154)
- [index.ts:1-111](file://supabase/functions/oss-sts/index.ts#L1-L111)
- [schema.sql:1-90](file://supabase/schema.sql#L1-L90)
- [buildMode.ts:1-8](file://src/buildMode.ts#L1-L8)

## 使用示例
以下为常见场景的实现要点与路径指引（不直接粘贴代码）：

- 新建项目并保存到云端
  - 入口：projectStore.newProject
  - 行为：构造 ProjectRecord，调用 upsertProjectToCloud 写入云端 projects 表
  - 参考路径：[projectStore.ts:149-182](file://src/store/projectStore.ts#L149-L182)、[cloudSync.ts:125-135](file://src/sync/cloudSync.ts#L125-L135)

- 自动保存项目
  - 入口：projectStore.saveNow
  - 行为：收集 nodes/edges/viewport，根据登录态选择云端或本地保存
  - 参考路径：[projectStore.ts:196-228](file://src/store/projectStore.ts#L196-L228)、[cloudSync.ts:125-135](file://src/sync/cloudSync.ts#L125-L135)

- 加载最近项目
  - 入口：projectStore.init
  - 行为：调用 syncProjectList 获取列表，选择最新项目加载
  - 参考路径：[projectStore.ts:81-117](file://src/store/projectStore.ts#L81-L117)、[cloudSync.ts:152-159](file://src/sync/cloudSync.ts#L152-L159)

- **更新** 上传媒体文件到云端（带进度）
  - 入口：uploadStore.runCloudUpload
  - 行为：调用 runAssetCloudUpload，自动处理分片上传和进度回调
  - 参考路径：[uploadStore.ts:27-74](file://src/store/uploadStore.ts#L27-L74)、[assetCloudUpload.ts:15-44](file://src/sync/assetCloudUpload.ts#L15-L44)

- **更新** 上传媒体文件到 OSS（带进度）
  - 入口：ossClient.uploadAssetToOssWithProgress
  - 行为：动态导入 ossClientImpl，根据文件大小自动选择分片或直接上传，提供进度回调
  - 参考路径：[ossClient.ts:22-32](file://src/sync/ossClient.ts#L22-L32)、[ossClientImpl.ts:77-100](file://src/sync/ossClientImpl.ts#L77-L100)

- 上传视频缩略图
  - 入口：ossClient.uploadThumbToOss
  - 行为：调用 thumbKey 生成 key 并上传
  - 参考路径：[ossClient.ts:34-40](file://src/sync/ossClient.ts#L34-L40)、[ossClientImpl.ts:102-109](file://src/sync/ossClientImpl.ts#L102-L109)

- 下载媒体文件
  - 入口：ossClient.downloadAssetFromOss
  - 行为：调用 get 并转换为 Blob
  - 参考路径：[ossClient.ts:42-48](file://src/sync/ossClient.ts#L42-L48)、[ossClientImpl.ts:120-131](file://src/sync/ossClientImpl.ts#L120-L131)

- 获取可访问 URL（私有桶）
  - 入口：ossClient.getOssUrl
  - 行为：调用 signatureUrl 生成带签名的 URL
  - 参考路径：[ossClient.ts:60-66](file://src/sync/ossClient.ts#L60-L66)、[ossClientImpl.ts:143-153](file://src/sync/ossClientImpl.ts#L143-L153)

- **更新** 同步素材元数据到云端
  - 入口：cloudSync.upsertAssetMetaToCloud
  - 行为：upsert 到 assets 表，记录 oss_key 与缩略图 key，返回布尔值表示成功与否
  - 参考路径：[cloudSync.ts:29-54](file://src/sync/cloudSync.ts#L29-L54)

- **更新** 自定义云端同步逻辑
  - 在 cloudSync.ts 中新增方法，封装业务逻辑（如批量同步、增量对比、重试机制）
  - 在 projectStore.ts 或业务组件中调用新接口，保持与现有流程一致
  - **新增** 使用 assetCloudUpload.ts 协调复杂的上传流程，利用其内置的错误处理和进度回调机制
  - 参考路径：[cloudSync.ts:1-169](file://src/sync/cloudSync.ts#L1-L169)、[assetCloudUpload.ts:1-45](file://src/sync/assetCloudUpload.ts#L1-L45)、[projectStore.ts:1-230](file://src/store/projectStore.ts#L1-L230)

## 结论
本方案通过 Supabase 与阿里云 OSS 的组合，实现了项目元数据的云端持久化与媒体文件的对象存储，配合行级安全与 STS 临时凭证，保障了数据安全与访问可控。系统在设计上清晰分层：同步编排、客户端封装、存储实现与认证管理各司其职，并通过构建目标开关灵活适配在线与局域网场景。

**更新总结** 本次更新显著增强了系统的可靠性和用户体验：
- **分片上传**：自动处理大文件传输，提高稳定性和效率
- **进度反馈**：实时显示上传进度，改善用户交互体验
- **错误处理**：完善的错误传播机制，确保数据一致性
- **状态管理**：智能的上传状态跟踪，支持并发任务和错误恢复

未来可在此基础上扩展增量同步、冲突合并、更细粒度的权限控制和高级分片策略，以满足更大规模协作需求。