...
## 更新摘要
**变更内容**
- 新增了桌面级迷你播放器（PiP）功能，支持浏览器窗口最小化后仍显示播放控制
- 增强了悬浮窗控制与主播放器的状态同步机制
- 完善了画中画API的兼容性检测和错误处理
- 优化了跨窗口通信和状态管理架构
...
## 简介
本仓库实现了一套完整的媒体播放体系，包含全局音频悬浮控制、沉浸式音频播放器（含频谱可视化、歌词显示、播放列表）、视频播放器（全屏、画中画、倍速、进度拖拽）以及统一的媒体协调器。**最新更新**：系统新增了桌面级迷你播放器（PiP）功能，利用Document Picture-in-Picture API在浏览器窗口最小化后仍以悬浮小窗显示播放控制，实现了跨窗口的状态同步和远程控制。系统通过单一音频元素作为"唯一播放引擎"，配合状态管理、歌单解析、资源加载与缓存机制，提供稳定且可扩展的媒体体验。
...
- **桌面级小窗 pipWindow**：利用 Document Picture-in-Picture API 创建独立小窗，实时同步播放状态与控制，支持跨窗口远程控制。
...
- GlobalPlayer 持有唯一 <audio> 元素，并通过 playerStore 暴露统一状态；其他 UI（悬浮窗、沉浸式播放器、**桌面级小窗**）只读状态并触发操作。
...
- **跨窗口通信**：通过 window.__pipCtrl 接口实现主页面与PiP小窗的状态同步和远程控制。

```mermaid
sequenceDiagram
participant U as "用户"
participant GP as "GlobalPlayer"
participant PW as "PiP Window"
...
Note over PW,PS : PiP小窗通过__pipCtrl接口<br/>定时同步状态并响应控制
```
...
- [pipWindow.ts:67-83](file://src/media/pipWindow.ts#L67-L83)
...
- 功能要点
  ...
  - **暴露 __pipCtrl 给 PiP 窗口进行远程控制**，包括播放控制、进度跳转、状态查询等功能。
- 关键流程
  ...
  - **通过 window.__pipCtrl 接口暴露播放器控制方法给PiP小窗**。
...
Ended -- 否 --> Idle["等待交互"]
Idle --> PipCtrl["暴露 __pipCtrl 接口<br/>供PiP小窗控制"]
```
...
### 桌面级迷你播放器（PiP）
- **功能特性**
  - 利用 Document Picture-in-Picture API 创建独立小窗，尺寸固定为 340×90 像素。
  - 提供完整的播放控制：上一首/下一首、播放/暂停、进度条拖拽、关闭窗口。
  - 实时状态同步：每500ms轮询主播放器状态，更新歌曲名、时间和播放按钮图标。
  - 跨窗口通信：通过 window.__pipCtrl 接口与主页面播放器进行双向通信。
  - 兼容性处理：isPipSupported() 检测API支持，不支持时静默降级。
- **技术实现**
  - 小窗生命周期管理：openPip/closePip/isPipOpen 函数管理窗口状态。
  - 事件处理：小窗内按钮点击事件映射到主播放器的控制方法。
  - 状态同步：startSync() 定时器持续同步播放状态到小窗UI。
  - 内存管理：窗口关闭时清理定时器和引用，防止内存泄漏。

```mermaid
sequenceDiagram
participant Main as "主页面"
participant PiP as "PiP小窗"
participant Store as "playerStore"
Note over Main,PiP : 建立跨窗口通信
Main->>Main : 暴露 window.__pipCtrl 接口
Main->>PiP : 打开小窗 (openPip)
loop 每500ms
PiP->>Main : 读取 __pipCtrl.getState()
Main->>Store : 获取当前播放状态
Store-->>Main : 返回 {track, playing, time, duration}
Main-->>PiP : 返回状态对象
PiP->>PiP : 更新UI显示
end
Note over PiP,Main : 用户在小窗中点击控制按钮
PiP->>Main : 调用 __pipCtrl.toggle()/next()/prev()/seekRatio()
Main->>Store : 执行相应的播放控制操作
Store-->>Main : 更新播放状态
```

**图表来源**
- [pipWindow.ts:27-91](file://src/media/pipWindow.ts#L27-L91)
- [GlobalPlayer.tsx:106-123](file://src/components/GlobalPlayer.tsx#L106-L123)
- [pipWindow.ts:107-133](file://src/media/pipWindow.ts#L107-L133)
...
- 状态同步：playerStore 集中管理播放状态，GlobalPlayer 与 AudioPlayer/VideoPlayer/**PiP小窗** 通过 store 方法驱动引擎。
...
PiPWindow --> PlayerStore : "通过__pipCtrl控制"
...
- 组件耦合
  ...
  - **PiP小窗与GlobalPlayer通过window.__pipCtrl接口松耦合通信**。
- 外部依赖
...
- **PiP小窗性能优化**
  - 定时同步间隔设置为500ms，平衡实时性与性能。
  - 小窗关闭时及时清理定时器和DOM引用，防止内存泄漏。
  - 跨窗口通信采用轻量级状态对象，避免大量数据传输。
...
- **PiP小窗问题**
  - 检查浏览器是否支持 documentPictureInPicture API。
  - 确认 window.__pipCtrl 接口是否正确暴露。
  - 验证小窗状态同步定时器是否正常运行。
  - 检查跨窗口通信是否正常，控制台是否有相关错误。
...
- [lyrics.ts:16-46](file://src/media/lyrics.ts#L16-L46)
- [lyrics.ts:129-167](file://src/media/lyrics.ts#L129-L167)
- [pipWindow.ts:19-35](file://src/media/pipWindow.ts#L19-L35)

## 结论
本媒体播放器体系以"单一音频引擎 + 多视图控制器"为核心，结合歌单解析、歌词显示、频谱可视化、资源缓存与互斥播放，提供了完整且可扩展的媒体体验。**最新更新**：新增的桌面级迷你播放器（PiP）功能进一步增强了用户体验，允许用户在浏览器窗口最小化后仍能便捷地控制音乐播放。通过清晰的职责划分与模块化设计，开发者可轻松定制 UI、扩展功能或集成新的媒体源。跨窗口通信机制为未来的多窗口协作场景奠定了坚实基础。
...
- 自定义配置
  ...
  - **PiP小窗配置**：可通过修改 pipWindow.ts 中的窗口尺寸、样式和控制按钮来自定义小窗外观和功能。
- 扩展开发
  ...
  - **扩展PiP功能**：通过修改 window.__pipCtrl 接口添加新的控制方法，在小窗中实现更多交互功能。
