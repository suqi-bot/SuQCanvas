...
- 图片查看器的加载、缩放、旋转（通过外部工具）、下载与 PSD 预览
...
- 下载与 PS 打开
  - 下载原始图片或 PSD；PSD 可下载后在 Photoshop 中打开
- 错误处理
  - 下载失败 toast 提示
...
## 结论
查看器组件以清晰的职责划分与统一的资源抽象实现了多类型内容的查看与播放。PDF 查看器借助 PDF.js 完成高质量渲染与导航；图片查看器提供灵活的缩放与下载能力；Markdown 查看器兼顾阅读与编辑；播放器页面提供沉浸式的多媒体体验。整体架构具备良好的可扩展性与健壮性，适合进一步定制与扩展。
