...
## 更新摘要
**变更内容**
- 修复了 App.tsx 中的用户认证状态管理 bug，将用户对象依赖转换为布尔值以避免 Supabase token 刷新导致的重复执行问题
- 优化了认证状态监听机制，防止因用户对象引用变化引起的副作用重复触发
- 增强了应用入口的状态判断逻辑，确保在 token 自动刷新时不会导致页面覆盖层被意外重新打开
...
**最新更新**：针对 Supabase token 自动刷新导致的重复执行问题进行了优化，通过将用户对象依赖转换为布尔值来避免不必要的副作用触发。
...
**图表来源**
- [src/App.tsx:20-51](file://src/App.tsx#L20-L51)
...
### 应用入口优化（App.tsx）
**最新更新**：修复了用户认证状态管理的重复执行问题

- **优化前的问题**：直接依赖 `user` 对象作为 useEffect 的依赖项，当 Supabase token 自动刷新时会创建新的用户对象引用，导致 effect 重复执行和页面覆盖层被意外重新打开。
- **优化后的方案**：将用户对象依赖转换为布尔值 `entered = Boolean(user || guest)`，只在认证状态真正改变时才触发相应的副作用。
- **效果**：避免了 token 刷新时的不必要重渲染，提升了应用性能和用户体验。
...
- **新增优化**：通过布尔值依赖替代对象引用依赖，避免 Supabase token 刷新导致的重复执行问题，提升应用响应性能。
...
- 令牌过期
  ...
  - **新优化**：token 刷新现在不会导致应用重复初始化，因为使用了布尔值依赖而非对象引用。
- 权限不足
...
**最新改进**：通过优化应用入口的状态依赖管理，解决了 Supabase token 自动刷新导致的重复执行问题，进一步提升了应用的稳定性和性能表现。
...
**章节来源**
- [src/App.tsx:25-47](file://src/App.tsx#L25-47)
- [src/components/AuthPage.tsx:61-73](file://src/components/AuthPage.tsx#L61-73)
- [src/store/authStore.ts:84-88](file://src/store/authStore.ts#L84-88)
- [src/store/authStore.ts:90-96](file://src/store/authStore.ts#L90-96)
- [src/store/authStore.ts:98-102](file://src/store/authStore.ts#L98-102)
- [src/store/projectStore.ts:38-41](file://src/store/projectStore.ts#L38-41)
- [src/sync/cloudSync.ts:148-165](file://src/sync/cloudSync.ts#L148-165)
- [src/components/AuthPage.tsx:75-130](file://src/components/AuthPage.tsx#L75-130)
- [src/sync/lanClient.ts:254-362](file://src/sync/lanClient.ts#L254-362)