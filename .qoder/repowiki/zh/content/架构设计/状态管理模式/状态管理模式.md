...
- [src/canvas/alignment/alignmentGuideStore.ts](file://src/canvas/alignment/alignmentGuideStore.ts)
...
## 更新摘要
**变更内容**
- 新增分组状态管理：在 canvasStore 中集成分组操作（成组、解散、删除分组及子孙），并通过 groups.ts 提供核心算法支持
- 新增对齐引导状态管理：新增 alignmentGuideStore.ts 作为瞬态状态存储，用于拖动过程中的参考线渲染
- 增强设置状态：settingsStore 新增对齐引导开关的持久化存储
- 更新画布状态：canvasStore 集成分组相关操作和约束逻辑
...
**更新** 本次更新新增了分组状态管理和对齐引导状态管理，增强了画布的层次化组织能力。

## 项目结构
SuQCanvas 将状态按"领域切片"拆分到多个 Store：
- 画布状态：canvasStore（节点、边、视口、撤销历史、对齐/层级、分组管理等）
...
- 设置状态：settingsStore（主题、对齐引导开关）
- **新增** 对齐引导状态：alignmentGuideStore（拖动过程中的瞬时参考线状态）
...
Alignment["alignmentGuideStore.ts"]
end
subgraph "分组功能"
Groups["groups.ts"]
GroupNode["GroupNode.tsx"]
GroupInspector["GroupInspectorSection.tsx"]
GroupToolbar["GroupToolbar.tsx"]
end
subgraph "持久化"
DB["db.ts (IndexedDB)"]
...
- [src/canvas/alignment/alignmentGuideStore.ts:1-27](file://src/canvas/alignment/alignmentGuideStore.ts#L1-L27)
- [src/canvas/groups.ts:1-205](file://src/canvas/groups.ts#L1-L205)

## 核心组件
- canvasStore：维护画布节点/边/视口、撤销/重做历史、复制粘贴、对齐、层级、资源清理、**分组管理**；通过防抖快照合并减少历史膨胀。
...
- settingsStore：主题切换与持久化、**对齐引导开关**。
- **新增** alignmentGuideStore：**瞬态状态管理**，仅用于拖动过程中保存当前需要渲染的参考线数据，不持久化且与主 store 解耦。
...
- [src/canvas/alignment/alignmentGuideStore.ts:13-20](file://src/canvas/alignment/alignmentGuideStore.ts#L13-L20)
...
- **新增** 分组功能通过 canvasStore 中的分组方法调用 groups.ts 中的算法函数，实现节点的层次化管理。
- **新增** 对齐引导通过独立的 alignmentGuideStore 管理瞬态状态，避免频繁更新导致的主 store 重渲染。
...
participant A as "alignmentGuideStore"
...
- [src/canvas/alignment/alignmentGuideStore.ts:22-26](file://src/canvas/alignment/alignmentGuideStore.ts#L22-L26)
...
- 关键算法：
  ...
  - **新增** 分组管理：groupSelected（成组）、dissolveGroup（解散）、deleteGroupWithDescendants（删除分组及子孙）。
- 副作用：插入元信息（创建者、时间），移除资产时清理相关节点和边。
- **新增** 分组约束：clampChildrenToParent 确保子节点在父框内，防止缩放溢出。
...
Schedule --> Apply
Apply --> CheckGroup{"是否涉及分组?"}
CheckGroup --> |是| Clamp["clampChildrenToParent()"]
CheckGroup --> |否| End(["状态更新完成"])
Clamp --> End
```
...
- [src/store/canvasStore.ts:464-546](file://src/store/canvasStore.ts#L464-L546)

### 分组状态管理
- **新增** 分组算法：groups.ts 提供分组相关的核心算法函数。
- 核心功能：
  - computeAbsolutePosition：计算节点在画布中的绝对坐标（考虑父子关系）。
  - collectDescendantIds：收集某分组的所有后代 id（含嵌套子分组及其成员）。
  - reparentToGroup：把若干节点重挂到新分组下，position 转为相对坐标。
  - dissolveGroup：解散分组，把直接子节点重指到新父级。
  - clampChildrenToParent：分组缩放后约束子节点在父框内。
  - normalizeGroupHierarchy：导入归一化，处理孤儿节点和排序。
- 集成方式：canvasStore 通过 import 调用这些算法函数，实现分组操作的完整流程。

**章节来源**
- [src/canvas/groups.ts:33-205](file://src/canvas/groups.ts#L33-L205)
- [src/store/canvasStore.ts:12-19](file://src/store/canvasStore.ts#L12-L19)
- [src/store/canvasStore.ts:464-546](file://src/store/canvasStore.ts#L464-L546)

### 对齐引导状态管理
- **新增** alignmentGuideStore：专门管理拖动过程中的对齐参考线状态。
- 设计原则：
  - 瞬态状态：仅在拖动过程中存在，拖动结束即清空。
  - 解耦设计：与画布主 store 分离，避免每帧更新导致 ReactFlow 整体重渲染。
  - 最小化更新：只有订阅该 store 的 <AlignmentGuides /> 组件会在参考线变化时重渲染。
- 状态结构：guides（当前参考线数组）、setGuides（设置参考线）、clear（清空参考线）。
- 使用场景：CanvasBoard 中拖动节点时计算对齐偏移，实时更新参考线显示。

**章节来源**
- [src/canvas/alignment/alignmentGuideStore.ts:1-27](file://src/canvas/alignment/alignmentGuideStore.ts#L1-L27)
- [src/canvas/CanvasBoard.tsx:91-123](file://src/canvas/CanvasBoard.tsx#L91-L123)
...
### 设置状态（settingsStore）
- 职责：主题切换与持久化（localStorage）、**对齐引导开关的持久化**。
- **新增** 对齐引导设置：showAlignmentGuides 控制拖动时是否显示智能吸附参考线，默认开启以提升用户体验。
...
- [src/store/settingsStore.ts:33-42](file://src/store/settingsStore.ts#L33-L42)
- [src/store/settingsStore.ts:55-63](file://src/store/settingsStore.ts#L55-L63)
...
## 依赖关系分析
- canvasStore 依赖 @xyflow/react 的变更处理函数，依赖 types 中的 SuqNode/SuqEdge 类型，**新增** 依赖 groups.ts 中的分组算法。
...
Player --> Canvas
Alignment["alignmentGuideStore.ts"] --> Canvas
App["App.tsx"] --> Auth
App --> Project
App --> UI
App --> LAN
...
- [src/canvas/alignment/alignmentGuideStore.ts:10-12](file://src/canvas/alignment/alignmentGuideStore.ts#L10-L12)
...
- **新增** 对齐引导性能优化：alignmentGuideStore 作为独立 store，避免拖动过程中频繁更新主 store 导致的整体重渲染。
- **新增** 分组操作优化：通过批量操作和算法优化，减少分组相关的数据转换和渲染开销。
...
- [src/canvas/alignment/alignmentGuideStore.ts:4-8](file://src/canvas/alignment/alignmentGuideStore.ts#L4-L8)
...
- **新增** 分组操作问题：检查 nodes 数组中是否正确设置了 parentId、extent 属性，确保父子关系正确建立。
- **新增** 对齐引导问题：确认 settingsStore.showAlignmentGuides 开关状态，检查 CanvasBoard 中的拖动事件处理逻辑。
...
## 结论
SuQCanvas 的状态管理以"领域切片 + 订阅联动"为核心，清晰划分了画布、项目、认证、UI、协作、播放器等职责。**本次更新新增了分组状态管理和对齐引导状态管理，进一步增强了画布的层次化组织和交互体验**。通过 IndexedDB 与云端双通道持久化、局域网实时同步与冲突收敛、以及多项性能优化（防抖、节流、缓存、GC），实现了高可用、可扩展的状态体系。建议在新增功能时遵循以下最佳实践：
...
- **新增** 瞬态状态分离：对于高频更新的临时状态（如对齐引导），考虑使用独立的 store 避免影响主状态的性能。
- **新增** 层次化数据管理：利用分组功能组织复杂画布结构，提高大型项目的可维护性。
...
协作同步 --> 编辑中 : "远端合并完成"
编辑中 --> 分组操作 : "groupSelected/dissolveGroup"
分组操作 --> 编辑中 : "分组状态更新"
编辑中 --> 对齐引导 : "拖动节点"
对齐引导 --> 编辑中 : "吸附完成"
```
