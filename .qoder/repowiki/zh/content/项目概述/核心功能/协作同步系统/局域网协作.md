...
- [src/components/LanProjectPanel.tsx](file://src/components/LanProjectPanel.tsx)
- [src/sync/lanHistory.ts](file://src/sync/lanHistory.ts)
...
## 更新摘要
**所做更改**
- 新增 LanProjectPanel 组件文档，提供桌面端局域网项目管理界面
- 更新连接管理部分，包含最近服务器历史记录功能
- 增强项目刷新机制，支持异步刷新和错误处理
- 添加 lanHistory.ts 模块的详细说明
- 更新架构图以反映新的组件结构
...
- 界面层：协作面板提供连接、断开、跟随、查看协作者与活动记录；桌面端项目面板提供局域网项目管理和刷新功能。
...
Client --> ProjectUI["项目面板<br/>LanProjectPanel"]
Client --> Store["协作状态<br/>lanStore"]
Client --> Sync["同步逻辑<br/>lanClient"]
Client --> History["历史记录<br/>lanHistory"]
Sync --> Store
UI --> Sync
ProjectUI --> Sync
ProjectUI --> History
...
- [src/components/LanProjectPanel.tsx:10-71](file://src/components/LanProjectPanel.tsx#L10-L71)
- [src/sync/lanHistory.ts:1-13](file://src/sync/lanHistory.ts#L1-L13)
...
- 项目面板：提供桌面端局域网项目管理，包括连接配置、项目刷新、历史服务器选择等功能。
- 历史记录：本地存储最近的服务器连接信息，支持快速选择和历史管理。
...
**新增功能** 桌面端项目面板提供了增强的局域网连接管理功能，包括最近服务器历史记录和项目刷新操作。

#### LanProjectPanel 组件
- **连接管理**：提供服务器地址输入、协作名称设置、连接状态显示
- **项目刷新**：支持异步刷新局域网项目列表，带进度反馈和错误处理
- **历史服务器**：自动保存最近使用的服务器连接信息，支持快速选择
- **状态管理**：集成 busy 状态检查，防止重复操作

#### 连接历史记录 (lanHistory.ts)
- **本地存储**：使用 localStorage 存储最近连接的服务器信息
- **去重管理**：自动去除重复的服务器地址，保持最多8条历史记录
- **时间排序**：按使用时间倒序排列，最新的连接记录在最前面
- **容错处理**：存储失败不影响连接功能，确保系统稳定性

#### 项目刷新机制
- **异步刷新**：refreshLanProjects 函数支持 AbortSignal，可取消长时间操作
- **状态订阅**：通过 useLanStore 订阅 remoteProjects 变化，自动完成刷新
- **超时保护**：10秒超时机制，防止无限等待
- **错误处理**：详细的错误消息反馈，包括网络异常和超时情况

```mermaid
flowchart TD
Connect["连接建立"] --> SaveConfig["保存连接配置"]
SaveConfig --> Remember["记录到历史服务器"]
Remember --> History["显示历史服务器列表"]
History --> QuickSelect["快速选择历史服务器"]
QuickSelect --> Connect
Connect --> Refresh["项目刷新"]
Refresh --> Status["显示刷新状态"]
Status --> Success["刷新成功"]
Status --> Error["刷新失败"]
Success --> Update["更新项目列表"]
...
- [src/components/LanProjectPanel.tsx:10-71](file://src/components/LanProjectPanel.tsx#L10-L71)
- [src/sync/lanHistory.ts:1-13](file://src/sync/lanHistory.ts#L1-L13)
- [src/sync/lanClient.ts:778-797](file://src/sync/lanClient.ts#L778-L797)
...
- 利用历史服务器功能快速连接到常用协作环境。
- 使用项目刷新功能及时获取最新的共享项目列表。
...
- 客户端依赖：
  ...
  - LanProjectPanel 依赖 lanHistory 管理连接历史，提供增强的项目管理功能。
- 服务端依赖：
...
LanProjectPanel["LanProjectPanel.tsx"] --> LanClient
LanProjectPanel --> LanHistory["lanHistory.ts"]
...
- [src/components/LanProjectPanel.tsx:1-71](file://src/components/LanProjectPanel.tsx#L1-L71)
- [src/sync/lanClient.ts:1-1313](file://src/sync/lanClient.ts#L1-L1313)
...
- 历史记录优化：
  - 限制最多8条历史记录，避免 localStorage 过度增长。
  - 自动去重和排序，提高用户体验。
...
- 项目刷新失败：
  - 检查网络连接状态，确保已建立 WebSocket 连接。
  - 查看刷新超时错误，可能是服务器响应缓慢或网络不稳定。
  - 使用历史服务器功能验证连接配置是否正确。
...
- [server/lan-server.mjs:773-823](file://server/lan-server.mjs#L773-L823)
- [src/components/LanProjectPanel.tsx:25-33](file://src/components/LanProjectPanel.tsx#L25-L33)

## 结论
SuQCanvas 的局域网协作通过 WebSocket 房间机制实现了高效的多人实时协作，结合并集合并、显式删除与墓碑机制保障了数据一致性；素材传输采用分片与 HTTP Range 流式拉取，显著优化了大文件在网络中的传输效率与用户体验。完善的断线重连与维护任务确保了系统的稳定性与资源利用率。新增的桌面端项目面板和历史记录功能进一步提升了用户体验，使局域网协作更加便捷和高效。按照本文的部署与排障建议，可在局域网环境中快速搭建并稳定运行协作服务。
...
- 桌面端使用：
  - 使用 LanProjectPanel 组件进行局域网项目管理，支持历史服务器快速连接。
  - 项目刷新功能提供实时的项目列表更新，支持异步操作和错误处理。
