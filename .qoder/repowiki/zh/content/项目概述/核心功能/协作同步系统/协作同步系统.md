...
- [src/desktop/DesktopCloudPanel.tsx](file://src/desktop/DesktopCloudPanel.tsx)
- [src/desktop/DesktopCloudSaveButton.tsx](file://src/desktop/DesktopCloudSaveButton.tsx)
- [src/desktop/cloudClient.ts](file://src/desktop/cloudClient.ts)
- [src/desktop/cloudProjects.ts](file://src/desktop/cloudProjects.ts)
- [src/desktop/saveCloudProject.ts](file://src/desktop/saveCloudProject.ts)
- [src/desktop/cloudStorage.ts](file://src/desktop/cloudStorage.ts)
- [src/store/projectStore.ts](file://src/store/projectStore.ts)
</cite>

## 更新摘要
**变更内容**
- 新增桌面应用云端保存功能，支持本地项目与云端项目的双向同步
- 增强局域网同步功能，优化断线重连和资产传输机制
- 完善桌面应用的 Supabase 集成和对象存储上传能力
- 新增桌面端项目管理和云端保存界面组件
...
- **桌面应用云端保存**：独立的桌面端云端同步流程，支持本地项目与云端项目的关联、版本控制和冲突解决。
...
- **桌面应用云端保存**：src/desktop/ 目录下的独立云端同步实现
...
subgraph "桌面应用"
F["DesktopCloudPanel.tsx<br/>云端项目管理界面"]
G["DesktopCloudSaveButton.tsx<br/>云端保存按钮"]
H["cloudClient.ts<br/>桌面云端客户端"]
I["cloudProjects.ts<br/>云端项目管理"]
J["saveCloudProject.ts<br/>云端保存逻辑"]
K["cloudStorage.ts<br/>云端存储操作"]
end
subgraph "服务器端"
...
**图表来源**
- [src/sync/lanClient.ts:1-1290](file://src/sync/lanClient.ts#L1-L1290)
...
- [src/desktop/DesktopCloudPanel.tsx:1-136](file://src/desktop/DesktopCloudPanel.tsx#L1-L136)
- [src/desktop/DesktopCloudSaveButton.tsx:1-109](file://src/desktop/DesktopCloudSaveButton.tsx#L1-L109)
- [src/desktop/cloudClient.ts:1-25](file://src/desktop/cloudClient.ts#L1-L25)
- [src/desktop/cloudProjects.ts:1-88](file://src/desktop/cloudProjects.ts#L1-L88)
- [src/desktop/saveCloudProject.ts:1-84](file://src/desktop/saveCloudProject.ts#L1-L84)
- [src/desktop/cloudStorage.ts:1-59](file://src/desktop/cloudStorage.ts#L1-L59)
...
- **桌面应用云端保存**：独立的云端同步流程，支持本地项目与云端项目的关联、版本控制、冲突检测和原子性更新。
...
## 架构总览
协作系统由"客户端—中继—云端"三层组成，新增桌面应用云端保存层：
...
- **桌面应用**：独立的云端同步流程，支持本地项目与云端项目的双向同步，具备版本控制和冲突解决机制。
...
- [src/desktop/saveCloudProject.ts:18-84](file://src/desktop/saveCloudProject.ts#L18-L84)
...
### 桌面应用云端保存功能
**新增** 桌面应用提供了独立的云端同步功能，支持与在线版服务的无缝集成：

- **云端项目管理界面**：DesktopCloudPanel 组件提供完整的云端项目管理工作台，支持登录、项目列表查看、项目下载等功能。
- **云端保存按钮**：DesktopCloudSaveButton 组件集成到编辑器工具栏，支持将本地项目保存到云端或更新现有云端项目。
- **独立云端客户端**：cloudClient.ts 提供独立的 Supabase 客户端实例，与主应用的用户会话隔离，确保数据安全。
- **项目关联机制**：通过 desktopCloudLinks 表记录本地项目与云端项目的关联关系，支持版本控制和冲突检测。
- **原子性保存流程**：saveCloudProject.ts 实现了完整的云端保存流程，包括素材上传、元数据同步、项目更新等步骤。

```mermaid
sequenceDiagram
participant UI as "桌面UI"
participant CP as "云端面板"
participant SP as "保存按钮"
participant SCP as "保存逻辑"
participant CS as "云端存储"
UI->>CP : 打开云端项目管理
CP->>CS : 登录并获取项目列表
UI->>SP : 点击保存到云端
SP->>SCP : 触发保存流程
...
**图表来源**
- [src/desktop/DesktopCloudPanel.tsx:11-136](file://src/desktop/DesktopCloudPanel.tsx#L11-L136)
- [src/desktop/DesktopCloudSaveButton.tsx:9-109](file://src/desktop/DesktopCloudSaveButton.tsx#L9-L109)
- [src/desktop/saveCloudProject.ts:18-84](file://src/desktop/saveCloudProject.ts#L18-L84)
...
- **桌面应用版本控制**：通过 updated_at 字段进行乐观锁控制，防止并发修改导致的数据覆盖。
...
- [src/desktop/saveCloudProject.ts:32-81](file://src/desktop/saveCloudProject.ts#L32-L81)
...
- **桌面应用**：独立的云端同步模块，与主应用通过接口隔离，支持独立部署和配置。
...
DCP["DesktopCloudPanel.tsx"] --> DCC["cloudClient.ts"]
DCS["DesktopCloudSaveButton.tsx"] --> DSC["saveCloudProject.ts"]
DSC --> DCS["cloudStorage.ts"]
DCC --> SUP["Supabase"]
DCS --> OSS["OSS"]
```
...
- **桌面应用优化**：大文件分块上传（10MB+ 使用分块上传），STS 凭证缓存减少认证开销。
...
- [server/lan-server.mjs:107-205](file://server/lan-server.mjs#L107-L205)
- [src/desktop/cloudStorage.ts:39-59](file://src/desktop/cloudStorage.ts#L39-L59)
...
- **桌面应用云端保存问题**
  - 登录状态失效：检查桌面应用独立会话存储 sq:desktop-cloud-auth。
  - 云端项目冲突：确认 updated_at 版本匹配，必要时另存为新项目。
  - 素材上传失败：检查 OSS 临时凭证有效性，确认网络连接稳定。
...
- [src/sync/ossClientImpl.ts:20-72](file://src/sync/ossClientImpl.ts#L20-L72)
- [src/desktop/saveCloudProject.ts:24-35](file://src/desktop/saveCloudProject.ts#L24-L35)

## 结论
SuQCanvas 的协作同步系统通过"客户端—中继—云端"的分层设计，实现了高效的局域网多人协作与可靠的云端同步。**新增的桌面应用云端保存功能**进一步增强了系统的完整性和用户体验：
- 明确的冲突解决策略（并集合并+墓碑机制）与显式删除传播。
- 高性能的资产传输（分片+HTTP Range）与维护任务保障。
- 灵活的状态管理与丰富的协作特性（光标、视图、编辑活动）。
- 云端与本地数据源的无缝切换，满足离线与在线场景。
- **桌面应用独立云端同步**：支持本地项目与云端项目的双向同步，具备完善的版本控制和冲突解决机制。
...
- **桌面应用配置**
  - 安装包需包含在线版服务配置（VITE_SUPABASE_URL、VITE_SUPABASE_ANON_KEY、VITE_OSS_*）。
  - 首次使用需在「在线项目」面板登录在线版账号。
  - 本地项目与云端项目通过 desktopCloudLinks 表关联，支持版本控制。
...
- [src/desktop/cloudClient.ts:3-13](file://src/desktop/cloudClient.ts#L3-L13)