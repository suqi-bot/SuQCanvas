  ...
  async function start() {
    ...
    try {
      ...
      if (mode === 'ai') {
        ...
        if (typeof workflow[image.node]?.inputs[image.input] !== 'string') throw new Error('请先在 AI 生图设置中配置拆图工作流及原图输入')
        if (negative.trim() && !settings.splitNegativeBinding) throw new Error('请先绑定拆图工作流的反向提示词输入')
      ...
      if (blob.size > 32 * 1024 * 1024 && mode === 'ai') throw new Error('AI 拆图原图不能超过 32MB')
      if (useProjectStore.getState().projectId !== projectId || useProjectStore.getState().busy || !useCanvasStore.getState().nodes.some((n) => n.id === nodeId)) throw new Error('项目已切换或原图已删除，请重新打开拆图')
      ...
      target.data.label = mode === 'grid' ? '网格拆图' : 'AI 分层拆图'
      if (mode === 'ai') target.data.ai = { ...target.data.ai!, provider: 'comfy', serviceUrl: settings.comfyUrl,
        ...
        prompt: prompt.trim() || '分层拆图', parameterSource: 'original' }
      useCanvasStore.getState().addNodes([target])
      if (mode === 'grid') void generateGridNode(target.id, blob, grid)
      else void generateAiNode(target.id, prompt.trim() || '分层拆图', blob)
      close()
    } catch (reason) { setError(reason instanceof Error ? reason.message : String(reason)) }
    finally { setBusy(false) }
  }
  return createPortal(<section role="dialog" aria-label="拆图" className="nodrag nopan fixed right-4 top-20 z-[80] max-h-[80vh] w-96 max-w-[calc(100vw-32px)] overflow-auto rounded-xl border border-edge2 bg-panel p-4 text-main shadow-2xl" onKeyDown={(e) => { e.stopPropagation(); if (e.key === 'Escape') close() }} onPointerDown={(e) => e.stopPropagation()}>
    <div className="flex items-center justify-between"><h2>拆图</h2><button aria-label="关闭拆图" onClick={close}>×</button></div>
    <div className="my-3 flex gap-2">{(['grid', 'ai'] as const).map((value) => <button key={value} className={`rounded px-3 py-2 text-xs ${mode === value ? 'bg-sky-600 text-white' : 'bg-hover'}`} onClick={() => setMode(value)}>{value === 'grid' ? '网格裁切' : 'AI 分层'}</button>)}</div>
    <div className="relative mx-auto w-fit max-w-full">
      <img src={url} alt="拆图预览" className="max-h-52 max-w-full" onLoad={(e) => setDimensions({ width: e.currentTarget.naturalWidth, height: e.currentTarget.naturalHeight })} />
    ...
    {mode === 'grid' ? <div className="my-3 grid grid-cols-2 gap-3">{([['rows', '行数'], ['columns', '列数'], ['gap', '间距（像素）'], ['margin', '外边距（像素）']] as const).map(([key, label]) => <label key={key} className="text-xs">{label}<input type="number" min={key === 'rows' || key === 'columns' ? 1 : 0} step={1} className="mt-1 w-full rounded border border-edge2 bg-panel p-2" value={grid[key]} onChange={(e) => setGrid({ ...grid, [key]: Number(e.target.value) })} /></label>)}</div>
      : <div className="my-3 space-y-3 text-xs"><p className="text-mid">原图发送至已配置的 ComfyUI，输出图层会作为独立图片加入画布。需要先配置分层工作流。</p>
        <button className="text-sky-500" onClick={() => useAiStore.getState().setSettingsOpen(true)}>配置 AI 拆图工作流</button>
        <label className="block">拆图需求<textarea disabled={!splitPromptBinding} className="mt-1 w-full rounded border border-edge2 bg-panel p-2 disabled:opacity-40" value={prompt} onChange={(e) => setPrompt(e.target.value)} /></label>
        {!splitPromptBinding && <p className="text-mid">未绑定正向提示词，将按工作流自身的分层配置执行。</p>}
        <label className="block">反向提示词<textarea disabled={!splitNegativeBinding} placeholder={splitNegativeBinding ? '不希望出现的内容' : '需要先在拆图设置中绑定反向输入'} className="mt-1 w-full rounded border border-edge2 bg-panel p-2 disabled:opacity-40" value={negative} onChange={(e) => setNegative(e.target.value)} /></label>
      </div>}
    ...
    <button disabled={busy || (mode === 'grid' && (!!gridError || !dimensions.width))} className="rounded bg-sky-600 px-3 py-2 text-sm text-white disabled:opacity-40" onClick={() => void start()}>{busy ? '准备中…' : mode === 'grid' ? '开始裁切' : '开始后台拆图'}</button>
